	...
	if (isset($_COOKIE['agencysearch']) && (isset($_GET['reset']) || isset($_POST['submitsearch']))) {
   	...
   	if (isset($_POST['submitsearch'])) { // Handle the form.
   		if(agency_account_type() == 'client' && is_active()) { // check if user is logged in and approved
			...
			echo '<br clear="all" />You must be approved as a Client before you may use the advanced search function</div>';
   		}
	}
	echo '<div style="margin-top:-40px; margin-left:170px; width:500px; position:absolute">';
	...
	if(isset($sql) && isset($sql_count)) {
		...
		while($row = sql_fetchrow($result)) {
			...
			echo '" /><br />' .
				...
				'<a href="./ajax/compcard_mini.php?u=' . $uid . '&height=400&amp;width=450" class="thickbox">View CompCard</a><br />' .
		...
		if($numpages>1){
		 ...
		 echo '<br /><a href="clienthome.php?mode=search" class="AGENCY_graybutton">new search</a>';
